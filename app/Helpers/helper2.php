...
if (!function_exists('getProfileImage')) {
    ...
    {
        // Extract only the first two letters of the name
        $initials = strtoupper(substr(str_replace(' ', '', $name), 0, 2));

        if (\Illuminate\Support\Facades\File::exists($path)) {
            return asset($path);
        } else {
            return 'https://ui-avatars.com/api/?background=random&name=' . $initials;
        }
    }
}
...
if (!function_exists('theme')) {
    function theme($fileName)
    {
        ...
        if (View::exists($path)) {
            ...
            return 'frontend.acslmstheme' . '.' . $fileName;
        }
