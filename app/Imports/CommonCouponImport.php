...
class CommonCouponImport implements ToCollection, WithHeadingRow, WithStyles
{
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            $row['start_date'] = Carbon::createFromFormat('d/m/Y', Arr::get($row, 'start_date'))->format('Y-m-d');
            $row['end_date'] = Carbon::createFromFormat('d/m/Y', Arr::get($row, 'end_date'))->format('Y-m-d');
