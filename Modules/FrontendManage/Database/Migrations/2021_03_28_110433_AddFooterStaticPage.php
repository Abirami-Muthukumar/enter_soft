...
class AddFooterStaticPage extends Migration
{
    ...
    public function up()
    {
        ...
        FrontPage::withoutEvents(function () {
            ...
            $acsedu = FrontPage::where('slug', 'acsedu-for-business')->first();
            if (!$acsedu) {
                DB::table('front_pages')->insert([
                    'name' => 'Learn with acsLMS',
                    'title' => 'Learn with acsLMS',
                    'sub_title' => 'Learn with acsLMS',
            ...
            $check6 = FooterWidget::where('slug', 'acsedu-for-business')->first();
            if ($check6) {
                $check6->slug = $acsedu->slug;
                $check6->page_id = $acsedu->id;
                $check6->is_static = $acsedu->is_static;
                $check6->save();
            }


            $teach = FrontPage::where('slug', 'teach-on-acsedu')->first();
            if (!$teach) {
                DB::table('front_pages')->insert([
                    'name' => 'Teach on acsEdu',
                    'title' => 'Teach on acsEdu',
                    'sub_title' => 'Teach on acsEdu',
            ...
            $check7 = FooterWidget::where('slug', 'teach-on-acsedu')->first();
            if ($check7) {
