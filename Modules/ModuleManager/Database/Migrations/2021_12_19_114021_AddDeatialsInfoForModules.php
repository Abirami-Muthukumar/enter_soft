...
//        DB::statement('ALTER TABLE `modules` CHANGE `details` `details` TEXT  NULL DEFAULT NULL;');
        ...
        if ($module) {
            $module->details = 'Add Tax functionality in acsLMS System. Admin can set tax percentage. Admin can Add country based tax percentage. When students buy any Item, the Tax will be applied. Tax can enable/disable/change anytime.';
            $module->save();
        }

        $module = Module::where('name', 'WhatsappSupport')->first();
        if ($module) {
            $module->details = 'WhatsApp Chat Support Module ready for acsLS that will help you to option to Non-stop messaging with clients on your website. This is one of the best ways to connect and interact with your customers, you can offer support directly as well as build trust and increase customer loyalty';
            $module->save();
        }
